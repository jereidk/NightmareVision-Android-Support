...
class ConfusionModifier extends NoteModifier
{
	...
	{
		if (!note.isSustainNote) note.angle = (getValue(player) + getSubmodValue(catKey('confusion', note.noteData), player) + getSubmodValue(catKey('note', note.noteData, 'Angle'), player));
	}

	override function updateReceptor(beat:Float, receptor:StrumNote, pos:Vector3,
			player:Int) receptor.angle = (getValue(player) + getSubmodValue(catKey('confusion', receptor.noteData), player) + getSubmodValue(catKey('receptor', receptor.noteData, 'Angle'), player));
